...
public class Route {
	...
	public int mode=mode_client;//1?????????,2?????????
							...
							if(mode==1){
								if(!setedTable.contains(remote_clientId)){
									...
									if(clientControl.getClientId_real()==-1){
										...
										//#MLog.println("????????????clientId "+remote_clientId);
									}else {
										if(clientControl.getClientId_real()!=remote_clientId){
											//#MLog.println("?????????????????????clientId "+sType+" "+clientControl.getClientId_real()+" new: "+remote_clientId);
